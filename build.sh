...
if [ ! -e "$__msbuildpath" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to restore MSBuild. Try importing certificates using 'mozroots --sync --import'."
        exit 1
    fi
fi

# execute the build
echo mono $__msbuildpath "$__buildproj" /nologo /verbosity:minimal "/fileloggerparameters:Verbosity=diag;LogFile=$__buildlog"
mono $__msbuildpath "$__buildproj" /nologo /verbosity:minimal "/fileloggerparameters:Verbosity=diag;LogFile=$__buildlog"
$BUILD
BUILDERRORLEVEL=$?
